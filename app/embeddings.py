        ...
        with self.lock:
            if self.model is None:
                ...
                if not self.cfg.embedding_model_path or not path.is_dir():
                    raise ValueError(
                        "임베딩 모델 폴더를 확인하세요: LOCAL_LLM_EMBEDDING_MODEL_PATH"
                    )
                try:
                    from sentence_transformers import SentenceTransformer
                except ImportError as exc:
                    raise RuntimeError(
                        "pip install -r requirements-embeddings.txt 실행이 필요합니다"
                    ) from exc
            ...
            prefix = (
                self.cfg.embedding_query_prefix
                if query
                else self.cfg.embedding_document_prefix
            )
...
class HybridRetriever:
    def __init__(self, memory, encoder=None):
        ...
        with memory.db.connect() as conn:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS embeddings (source TEXT, source_id INTEGER, model TEXT, digest TEXT, vector TEXT, PRIMARY KEY(source, source_id, model))"
            )
    ...
    def reindex(self, force=False):
        ...
        with self.lock, self.memory.db.connect() as conn:
            if force:
                conn.execute(
                    "DELETE FROM embeddings WHERE model=?", (self.encoder.identity,)
                )
            rows = conn.execute(
                "SELECT source, source_id, title, body FROM memory_fts"
            ).fetchall()
            changed = 0
            for row in rows:
                text = row["title"] + "\n" + row["body"]
                ...
                old = conn.execute(
                    "SELECT digest FROM embeddings WHERE source=? AND source_id=? AND model=?",
                    (row["source"], row["source_id"], self.encoder.identity),
                ).fetchone()
    ...
    def search(self, query, limit=5):
        ...
        with self.memory.db.connect() as conn:
            rows = conn.execute(
                "SELECT f.source, f.source_id, f.title, f.body, e.vector FROM memory_fts f JOIN embeddings e ON f.source=e.source AND f.source_id=e.source_id WHERE e.model=?",
                (self.encoder.identity,),
            ).fetchall()
        ...
        for row in rows:
            raw = dict(row)
            stored_vector = json.loads(raw.pop("vector"))
            enriched = self.memory.enrich_result(raw)
            if enriched is None:
                continue
            enriched["similarity"] = cosine(vector, stored_vector)
            semantic.append(enriched)
        semantic.sort(key=lambda item: item["similarity"], reverse=True)
        ...
        for ranking in (lexical, semantic):
            for index, row in enumerate(ranking):
                ...
                quality = float(row.get("memory_quality", 1.0))
                item["hybrid_score"] = item.get("hybrid_score", 0.0) + quality / (
                    60 + index + 1
                )
